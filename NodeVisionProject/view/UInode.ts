class UINode{
    ...
    private ctrlnode: Ctrlnode;
    private ctrlattribute: Ctrlattribute;
    private ctrlpreferencepopup: Ctrlpreferencepopup;
    private attrs = new Array<Attribute>();
    ...
    public setui(node: NVNode, nodeModal: UIModal) {
        //MODAL HEAD
        ...
        var h3 = new Elem('h4').e; h3.innerHTML = 'Node: ';
        var h3_name = new Elem('h4').e; h3_name.innerHTML = this.node.name;
        d_col3.appendChild(h3); d_col4.appendChild(h3_name);

        //Update node name
        h3_name.ondblclick = () => {
            var input = <HTMLInputElement>new Elem('input', 'form-control col-md-10', [new Attribute('typename', 'text'), new Attribute('value', this.node.name)]).e;
            jQuery(h3_name).replaceWith(input);
            input.focus();
            input.onblur = () => {
                this.node.name = input.value;
                var res = this.ctrlnode.updateModel(new request(Action.update, this.node));
        ...
        b_child.onclick = (e) => {
            var res = this.ctrlgraph.updateModel(new request(Action.update, this.node));
            this.popup(res);
        };
        ...
        b_delete_child.onclick = (e) => {
            var res = this.ctrlgraph.updateModel(new request(Action.delete, this.node));
            this.popup(res);
        };
        //bouton delete node
        ...
        b_attribut.onclick = (e) => {
            var att = <Elem<HTMLElement>>this.ctrlattribute.updateModel(new request(Action.update)).ui;
    ...
    public accordion_user() {
        var div_user = new Elem('div','row');

        var group = new Elem('div', 'btn-group');
        var user = new Elem('div', 'btn-group');
        var btn_group = new Elem('button', 'btn btn-default  dropdown-toggle', [new Attribute('data-toggle', 'dropdown'), new Attribute('aria-haspopup', 'true'), new Attribute('aria-expanded', 'false')]); btn_group.e.innerHTML = "Groups";
        var span_group = new Elem('span', 'caret');
        var ul_group = new Elem('ul', 'dropdown-menu');
        var btn_user = new Elem('button', 'btn btn-default  dropdown-toggle', [new Attribute('data-toggle', 'dropdown'), new Attribute('aria-haspopup', 'true'), new Attribute('aria-expanded', 'false')]); btn_user.e.innerHTML = "Users";
        ...
        */
        ...
        var thead = new Elem('thead'); var trhead = new Elem('tr').e; thead.appendChild(trhead);
        var tbody = new Elem('tbody');
        ['Name (group,user)', 'Write', 'Adminstration','supprimer'].forEach(th_name => {
            var th = new Elem('th').e; th.innerHTML = th_name;
        ...
        //
        //});
        div_user.appendChild(
            accordion.appendChild(
                panel_default.appendChild([
                    panel_heading.appendChild(
                        h4.appendChild(
                            a.e).e).e,
                    panel_collapse.appendChild
                        (panel_body.appendChild([
                            group.e,user.e,
                            table.appendChild([
                                thead.e,
                                tbody.e]).e
